...
- 二维码登录、Cookie 导入、手机号/邮箱密码登录、登录态校验与退出登录基础闭环。
...
- `/album`、`/artists` 专辑/艺术家详情已接入（信息 + 歌曲/热门歌曲 + 播放）；`Track` 携带 `albumId`/`artistIds`，播放页「更多」菜单可跳转，专辑页可跳转歌手。
- 播放队列体验优化：在线歌单「秒播 + 后台解析」（只解析起始曲并立即播放，其余后台补齐）；队列增删改用 `ConcatenatingAudioSource` 原地操作不再整轮重建（消除改队列卡顿）；搜索点歌插入到当前曲之后并播放、不清空队列；「正在播放」队列持久化、重开恢复（停在暂停态）；网易云歌单/专辑/艺术家先出缓存、过期再刷新。
