...
| `app_data.now_playing.v1` | 「正在播放」队列快照（曲目 + 当前索引 + 进度），重开恢复用 |
| `app_data.netease.collection.<id>` | 网易云歌单/专辑/艺术家缓存（先出缓存、过期再刷新） |
| `flutter_secure_storage.netease.auth.cookie.v1` | 网易云登录 Cookie（密钥环加密，不可用时回退 `shared_preferences`） |
...
网易云登录 cookie 由 `SecureCookieStore` 管理：优先存入系统密钥环（`flutter_secure_storage`）做 at-rest 加密，密钥环不可用时回退到 `shared_preferences`；历史明文 cookie 会在读取时迁移进密钥环并清除明文副本。
...
lib/data/local/database/
  app_database.dart                schema、版本化迁移与所有数据访问
  app_database_factory*.dart       平台条件导入（原生 FFI / Web）
```
...
> 版本化迁移已接入：`databaseVersion` 定义当前版本（现为 1，由 `_onCreate` 建表），
> 新增 schema 时递增该版本并在 `app_database.dart` 的 `_migrations`（`{目标版本: SQL 脚本}`）
> 登记升级语句，`_onUpgrade` 按版本顺序执行（`migrationScriptsFor` 为可单测的纯函数）。
> 原 `migrations/*.sql` 快照文件已移除，schema 以 `_onCreate` + `_migrations` 为唯一来源。
